...
/*TRIGGERS*/
CREATE TRIGGER before_stud_update1 
...
drop trigger before_stud_update1;

/*TO SEE ALL THE TABLES*/
show tables;

/*CONCATENATION OF TWO COLUMNS*/
select concat(first_name , last_name),length(concat(first_name , last_name)) from student;
...
/*UPPER FUNCTION*/
select ucase("pakistan");

/*LOWER FUNCTION*/
select lcase("PAKISTAN");

/*LENGTH FUNCTION*/
SELECT length("karachi is the biggest city");
...
SELECT DATEDIFF("2017-06-25", "2017-06-15");
 ...
 where fees not in (select max(fees) from student);
 SELECT fees FROM STUDENT order by fees desc limit 1 offset 2;
...
DELIMITER //
CREATE PROCEDURE Getspecialstudent(
	IN studnumber VARCHAR(55)
)
BEGIN
	SELECT * 
 	FROM student
	WHERE seat_number = studnumber;
END //

DELIMITER ;

CALL Getspecialstudent("eb20103038");
...
insert into demo2 values("hasham","a");
insert into demo2 values("hasham","b");
insert into demo2 values("azhar","c");
insert into demo2 values("azhar","d");
insert into demo2 values("moin","e");
insert into demo2 values("moin","f");

/*TO MERGE TWO ROWS*/
select name,group_concat(book) from demo2 group by name;
